...
# Vectors from the LEGO Group protocol examples (Apache-2.0).
# fmt: off
PACK_CASES = (
    (
...
def test_encode_decode_roundtrip() -> None:
    data = b"Hello, World!"
    assert bytes(decode(encode(data))) == data
...
def test_pack_official_vectors() -> None:
    for data, expected in PACK_CASES:
        assert pack(data) == expected


def test_unpack_official_vectors() -> None:
    for expected, data in PACK_CASES:
        assert unpack(data) == expected
